<!DOCTYPE html>
<html lang="es">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="A gorgeous looking onepage site template suitable for photographers, visual artists, digital studios, freelancers that would like to showcase their portfolio beautifully.">
  <meta name="keywords" content="bootstrap 5, one page, responsive, photography, minimal, modern, gulp, sass">
  <meta name="author" content="elemis">
  <title>Tramita Visa</title>
  <link rel="shortcut icon" href="./assets/img/favicon.png">
  <link rel="stylesheet" href="./assets/css/plugins.css">
  <link rel="stylesheet" href="./assets/css/style-light.css">
  <link rel="stylesheet" href="./assets/css/colors/green.css">
 <link rel="stylesheet" href="./assets/css/preloader.css">
</head>


<body
  
  class="onepage" data-bs-spy="scroll" data-bs-target=".navbar-collapse .navbar-nav">
  <div class="content-wrapper">
    <header class="position-absolute top-0 start-0 w-100">
      <nav class="navbar navbar-expand-lg classic transparent ">
        <div class="container flex-lg-row flex-nowrap align-items-center">
          <div class="navbar-brand w-100">
            <a href="./index-light.html">
              <img src="/dev/assets/img/visa/logov.png" srcset="" alt="" height="250"/>
            </a>
          </div>
          
          <div class="navbar-collapse offcanvas offcanvas-nav offcanvas-start">
            <div class="offcanvas-header d-lg-none">
              <a href="./index-light.html"><img src="./assets/img/logo-dark.png" srcset="./assets/img/logo-dark@2x.png 2x" alt="" /></a>
              <button type="button" class="btn btn-circle btn-white btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
            </div>
            
            <div class="offcanvas-body d-flex flex-column h-100">
              
              <ul class="navbar-nav">
                <li class="nav-item"><a class="nav-link scroll active" href="#home">Home</a></li>
                <li class="nav-item"><a class="nav-link scroll" href="#about">Acerca de Nosotros</a></li>
                <li class="nav-item"><a class="nav-link scroll" href="#Proces">Proceso a seguir</a></li>
                <li class="nav-item"><a class="nav-link scroll" href="#gallery">Conoce</a></li>
                <li class="nav-item"><a class="nav-link scroll" href="#services">Servicios</a></li>
                <li class="nav-item"><a class="nav-link scroll" href="#contact">Contacto</a></li>
                
              <!-- /.navbar-nav -->

              <div class="d-lg-none mt-auto pt-6 pb-6 order-4">
                <a href="mailto:#" class="link-body">info@locus.com</a>
                <br /> 00 (123) 456 78 90 <br />
                <nav class="nav social mt-4">
                  <a href="#" class="btn btn-circle btn-dark btn-behance btn-sm"><i class="uil uil-behance"></i></a>
                  <a href="#" class="btn btn-circle btn-dark btn-facebook-f btn-sm"><i class="uil uil-facebook-f"></i></a>
                  <a href="#" class="btn btn-circle btn-dark btn-instagram btn-sm"><i class="uil uil-instagram"></i></a>
                </nav>
                <!-- /.social -->
              </div>
              <!-- /offcanvas-nav-other -->
            </div>
            <!-- /.offcanvas-body -->
          </div>
          <!-- /.navbar-collapse -->
          <div class="navbar-other w-100 d-flex ms-auto">
            <ul class="navbar-nav flex-row align-items-center ms-auto">
              <li class="nav-item">
                <nav class="nav social justify-content-end text-end">
                  <a href="#" class="btn btn-circle btn-dark btn-behance btn-sm"><i class="uil uil-behance"></i></a>
                  <a href="#" class="btn btn-circle btn-dark btn-facebook-f btn-sm"><i class="uil uil-facebook-f"></i></a>
                  <a href="#" class="btn btn-circle btn-dark btn-instagram btn-sm"><i class="uil uil-instagram"></i></a>
                </nav>
                <!-- /.social -->
              </li>
              <li class="nav-item d-lg-none">
                <button class="hamburger offcanvas-nav-btn"><span></span></button>
              </li>
            </ul>
            <!-- /.navbar-nav -->
            
          </div>
          <!-- /.navbar-other -->
          
        </div>
        <!-- /.container -->
      </nav>
      
      <!-- /.navbar -->
    </header>
    
    <section id="home">
      <div class="container vh-100 d-flex flex-column align-items-center justify-content-center" data-cues="slideInDown" data-group="hero">
        <h2 class="fs-16 ls-lg text-uppercase mb-5">Trámites de Visa</h2>
        <h1 class="display-1 mb-8">Servicio de Gestoria</h1>
        <span><a href="#Proces" class="btn btn-lg btn-dark scroll">Proceso a seguir</a></span>
      </div>
    </section>
    <section id="about">
      <div class="container pt-7 pb-15">
        <div class="card">
          <div class="card-body">
            <div class="row gx-md-8 gx-xl-10 gy-8">
              <div class="col-lg-5">
                <figure class="rounded"><img class="img-fluid" src="/dev/assets/img/visa/visa2.jpg"   srcset="" alt=""></figure>
              </div>
              <!-- /column -->
              <div class="col-lg-7">
                <h2 class="mb-3" align="center">Quienes somos</h2>
                <p class="lead mb-7" align="justify">Somos un equipo de expertos en la gestión de trámite de visa americana, con más de 8 años de experiencia apoyando a la gestión y consultoría de cualquier trámite migratorio relacionado con el trámite de visa Estadounidense, brindamos seguridad en nuestros procesos, contando con profesionales preparados para cada caso. Todos los días brindamos asesorías a todas las personas que están interesadas en tramitar su visa. Contamos con altas tazas de éxito en nuestros procesos. Nuestro trabajo está estrictamente apegado a la Ley Federal de Protección de Datos Personales en posesión de los particulares. Nosotros reforzamos tu perfil, ofreciendo un 90% de efectividad. Brindando seguridad y calidad en tiempo y forma de acuerdo a las necesidades de nuestros clientes </p>
               
                  </div>
                  <!-- /column -->
                </div>
                <!-- /.row -->
              </div>
              <!-- /column -->
            </div>
            <!-- /.row -->
          </div>
          <!--/.card-body -->
        </div>
        <!--/.card -->
      </div>
    </section>
    
    </section>
    <section id="gallery">
      <div class="container pt-7 pb-15">
        <div class="card">
          <div class="card-body">
            <h2 class="mb-3 text-center">Un poco de USA</h2>
            <p class="lead mb-10 text-center px-xl-12">Lugares emblematicos</p>
            <div class="grid grid-view">
              <div class="row gx-md-6 gy-6 isotope">
                <div class="item col-md-6 col-xl-4">
                  <figure class="overlay rounded">
                    <a href="./assets/img/photos/an1.jpg" data-glightbox="title: Antarctica" data-gallery="g-antarctica">
                      <img src="/dev/assets/img/visa/lg1.jpg" srcset="./assets/img/photos/an-th@2x.jpg 2x" alt="" />
                    </a>
                    <figcaption>
                      <h4 class="from-top h6 mb-0">Antarctica</h4>
                    </figcaption>
                  </figure>
                  <ul class="d-none">
                    <li><a href="./assets/img/photos/an2.jpg" data-glightbox="title: Antarctica" data-gallery="g-antarctica"></a></li>
                    <li><a href="./assets/img/photos/an3.jpg" data-glightbox="title: Antarctica" data-gallery="g-antarctica"></a></li>
                    <li><a href="./assets/img/photos/an4.jpg" data-glightbox="title: Antarctica" data-gallery="g-antarctica"></a></li>
                    <li><a href="./assets/img/photos/an5.jpg" data-glightbox="title: Antarctica" data-gallery="g-antarctica"></a></li>
                    <li><a href="./assets/img/photos/an6.jpg" data-glightbox="title: Antarctica" data-gallery="g-antarctica"></a></li>
                  </ul>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-xl-4">
                  <figure class="overlay rounded">
                    <a href="./assets/img/photos/au1.jpg" data-glightbox="title: Australia" data-gallery="g-australia">
                      <img src="/dev/assets/img/visa/lg2.jpg" srcset="./assets/img/photos/au-th@2x.jpg 2x" alt="" />
                    </a>
                    <figcaption>
                      <h4 class="from-top h6 mb-0">Australia</h4>
                    </figcaption>
                  </figure>
                  <ul class="d-none">
                    <li><a href="./assets/img/photos/au2.jpg" data-glightbox="title: Australia" data-gallery="g-australia"></a></li>
                    <li><a href="./assets/img/photos/au3.jpg" data-glightbox="title: Australia" data-gallery="g-australia"></a></li>
                    <li><a href="./assets/img/photos/au4.jpg" data-glightbox="title: Australia" data-gallery="g-australia"></a></li>
                    <li><a href="./assets/img/photos/au5.jpg" data-glightbox="title: Australia" data-gallery="g-australia"></a></li>
                    <li><a href="./assets/img/photos/au6.jpg" data-glightbox="title: Australia" data-gallery="g-australia"></a></li>
                  </ul>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-xl-4">
                  <figure class="overlay rounded">
                    <a href="./assets/img/photos/eg1.jpg" data-glightbox="title: Egypt" data-gallery="g-egypt">
                      <img src="/dev/assets/img/visa/lg3.jpg" srcset="./assets/img/photos/eg-th@2x.jpg 2x" alt="" />
                    </a>
                    <figcaption>
                      <h4 class="from-top h6 mb-0">Egypt</h4>
                    </figcaption>
                  </figure>
                  <ul class="d-none">
                    <li><a href="./assets/img/photos/eg2.jpg" data-glightbox="title: Egypt" data-gallery="g-egypt"></a></li>
                    <li><a href="./assets/img/photos/eg3.jpg" data-glightbox="title: Egypt" data-gallery="g-egypt"></a></li>
                    <li><a href="./assets/img/photos/eg4.jpg" data-glightbox="title: Egypt" data-gallery="g-egypt"></a></li>
                    <li><a href="./assets/img/photos/eg5.jpg" data-glightbox="title: Egypt" data-gallery="g-egypt"></a></li>
                    <li><a href="./assets/img/photos/eg6.jpg" data-glightbox="title: Egypt" data-gallery="g-egypt"></a></li>
                  </ul>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-xl-4">
                  <figure class="overlay rounded">
                    <a href="./assets/img/photos/sc1.jpg" data-glightbox="title: Scandinavia" data-gallery="g-scandinavia">
                      <img src="/dev/assets/img/visa/lg4.jpg" srcset="./assets/img/photos/sc-th@2x.jpg 2x" alt="" />
                    </a>
                    <figcaption>
                      <h4 class="from-top h6 mb-0">Scandinavia</h4>
                    </figcaption>
                  </figure>
                  <ul class="d-none">
                    <li><a href="./assets/img/photos/sc2.jpg" data-glightbox="title: Scandinavia" data-gallery="g-scandinavia"></a></li>
                    <li><a href="./assets/img/photos/sc3.jpg" data-glightbox="title: Scandinavia" data-gallery="g-scandinavia"></a></li>
                    <li><a href="./assets/img/photos/sc4.jpg" data-glightbox="title: Scandinavia" data-gallery="g-scandinavia"></a></li>
                    <li><a href="./assets/img/photos/sc5.jpg" data-glightbox="title: Scandinavia" data-gallery="g-scandinavia"></a></li>
                    <li><a href="./assets/img/photos/sc6.jpg" data-glightbox="title: Scandinavia" data-gallery="g-scandinavia"></a></li>
                  </ul>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-xl-4">
                  <figure class="overlay rounded">
                    <a href="./assets/img/photos/aa1.jpg" data-glightbox="title: Anatolia" data-gallery="g-anatolia">
                      <img src="/dev/assets/img/visa/lg5.jpg" srcset="./assets/img/photos/aa-th@2x.jpg 2x" alt="" />
                    </a>
                    <figcaption>
                      <h4 class="from-top h6 mb-0">Anatolia</h4>
                    </figcaption>
                  </figure>
                  <ul class="d-none">
                    <li><a href="./assets/img/photos/aa2.jpg" data-glightbox="title: Anatolia" data-gallery="g-anatolia"></a></li>
                    <li><a href="./assets/img/photos/aa3.jpg" data-glightbox="title: Anatolia" data-gallery="g-anatolia"></a></li>
                    <li><a href="./assets/img/photos/aa4.jpg" data-glightbox="title: Anatolia" data-gallery="g-anatolia"></a></li>
                    <li><a href="./assets/img/photos/aa5.jpg" data-glightbox="title: Anatolia" data-gallery="g-anatolia"></a></li>
                    <li><a href="./assets/img/photos/aa6.jpg" data-glightbox="title: Anatolia" data-gallery="g-anatolia"></a></li>
                  </ul>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-xl-4">
                  <figure class="overlay rounded">
                    <a href="./assets/img/photos/br1.jpg" data-glightbox="title: Brazil" data-gallery="g-brazil">
                      <img src="./assets/img/photos/br-th.jpg" srcset="./assets/img/photos/br-th@2x.jpg 2x" alt="" />
                    </a>
                    <figcaption>
                      <h4 class="from-top h6 mb-0">Brazil</h4>
                    </figcaption>
                  </figure>
                  <ul class="d-none">
                    <li><a href="./assets/img/photos/br2.jpg" data-glightbox="title: Brazil" data-gallery="g-brazil"></a></li>
                    <li><a href="./assets/img/photos/br3.jpg" data-glightbox="title: Brazil" data-gallery="g-brazil"></a></li>
                    <li><a href="./assets/img/photos/br4.jpg" data-glightbox="title: Brazil" data-gallery="g-brazil"></a></li>
                    <li><a href="./assets/img/photos/br5.jpg" data-glightbox="title: Brazil" data-gallery="g-brazil"></a></li>
                    <li><a href="./assets/img/photos/br6.jpg" data-glightbox="title: Brazil" data-gallery="g-brazil"></a></li>
                  </ul>
                </div>
                <!-- /.item -->
              </div>
              <!-- /.row -->
            </div>
            <!-- /.grid -->
          </div>
          <!--/.card-body -->
        </div>
        <!--/.card -->
      </div>
    </section>
  </section>
  <section id="Proces">
    <div class="container pt-7 pb-15">
      <div class="card">
        <div class="card-body">
          <div class="row gx-lg-8 gx-xl-12 gy-10 align-items-center">
            <div class="col-lg-6 position-relative order-lg-2">
              <div class="overlap-grid overlap-grid-2">
                <div class="item">
                  <figure class="rounded shadow"><img src="/dev/assets/img/visa/nueva3.jpg" srcset="" alt=""></figure>
                </div>
                <div class="item">
                  <figure class="rounded shadow"><img src="/dev/assets/img/visa/visa1.jpg"srcset="./assets/img/photos/s2@2x.jpg 2x" alt=""></figure>
                </div>
              </div>
            </div>
            <!--/column -->
            <div class="col-lg-6">
              <h2 class="mb-3" align="center">Proceso a seguir</h2>
              <p class="lead mb-7" align="justify">Llenar un cuestionario para conocer el perfil del tramitante para así hacer una estrategia en base al perfil. Envió de una fotografía del pasaporte para hacer el registro ante la Embajada Americana. El pago de Derechos de Visa para poder agendar sus citas. La primer cita es para foto y huellas y la segunda cita es para la entrevista consular.</p>
              <div class="d-flex flex-row mb-5">
                <div>
                  <div class=""><i class=""></i></div>
                </div>
                <div>
                  <h3 class="h4 mb-1" align="center">Que sigue</h3>
                  <p class="mb-0" align="justify">
                    Una vez descargado y completado el formato de cuestionario, envíalo a info@tramitalavisa.com para iniciar tu proceso y que nuestros expertos puedan generar la mejor estrategia de acuerdo a tu perfil.</p>
                </div>
              </div>

             
            <!--/column -->
          </div>
          <!--/.row -->
        </div>
        <!--/.card-body -->
      </div>
      <!--/.card -->
    </div>
  </section>
    <section id="services">
      <div class="container pt-7 pb-15">
        <div class="card">
          <div class="card-body">
            <div class="row gx-lg-8 gx-xl-12 gy-10 align-items-center">
              <div class="col-lg-6 position-relative order-lg-2">
                <div class="overlap-grid overlap-grid-2">
                  <div class="item">
                    <figure class="rounded shadow"><img src="/dev/assets/img/visa/nueva3.jpg" srcset="" alt=""></figure>
                  </div>
                  <div class="item">
                    <figure class="rounded shadow"><img src="/dev/assets/img/visa/visa1.jpg"srcset="./assets/img/photos/s2@2x.jpg 2x" alt=""></figure>
                  </div>
                </div>
              </div>
              <!--/column -->
              <div class="col-lg-6">
                <h2 class="mb-3" align="center">Servicios</h2>
                <p class="lead mb-7" align="justify">Reforzamos tu perfil junto con una estrategia en el proceso de consultoría para poder brindarte la mejor opción para que tu trámite sea exitoso. Te preparamos y asesoramos para que tu cita consular sea factible en cualquiera de las locaciones consulares disponibles. Te ofrecemos visa turística y visa de trabajo.</p>
                <div class="d-flex flex-row mb-5">
                  <div>
                    <div class=""><i class=""></i></div>
                  </div>
                  <div>
                    <h3 class="h4 mb-1">Analisis</h3>
                    <p class="mb-0">Se analiza la estructura de tu perfil para poder generar una estrategia para tu cita consular, completando el formulario DS-160.</p>
                  </div>
                </div>

                <div class="d-flex flex-row mb-5">
                  <div>
                    <div class=""><i class=""></i></div>
                  </div>
                  <div>
                    <h3 class="h4 mb-1">Asesoria</h3>
                    <p class="mb-0">Te asesoramos en todo el proceso de tu solicitud, incluye análisis de tu situación, estrategia y como mejorar la posibilidad de que te sea otorgada la visa.</p>
                  </div>
                </div>
                   <div class="d-flex flex-row mb-5">
                  <div>
                    <div class=""><i class=""></i></div>
                  </div>
                  <div>
                    <h3 class="h4 mb-1">Documentacion</h3>
                    <p class="mb-0">Listado de documentos indispensables y de soporte ligados a su realidad especifica.</p>
                  </div>
                </div>
                <div class="d-flex flex-row mb-5">
                  <div>
                  <div>
                    <h3 class="h4 mb-1">Agenda</h3>
                    <p class="mb-0">Programación de la cita para la entrevista de visa y para la toma de huellas, fotografía en el CAS para los países que aplica.</p>
                  </div>
                </div>
              </div>
              <!--/column -->
            </div>
            <!--/.row -->
          </div>
          <!--/.card-body -->
        </div>
        <!--/.card -->
      </div>
    </section>
    <section id="contact">
      <div class="container pt-7 pb-15">
        <div class="card">
          <div class="card-body">
            <h2 class="mb-3">Contactanos</h2>
            <p class="lead mb-10">Tienes alguna duda</p>
            <!--INFO-->
                    <!-- /column -->
                    
              <div class="col-lg-4">
                <div class="d-flex flex-row">
                  <div>
                    <div class="icon text-dark fs-30 me-5 mt-1"> <i class="si-ecommerce_shop-location"></i> </div>
                  </div>
                  <div>
                    <h5 class="mb-1">Address</h5>
                    <address>Moonshine St. 14/05 Light City, London, United Kingdom</address>
                  </div>
                </div>
                <div class="d-flex flex-row">
                  <div>
                    <div class="icon text-dark fs-30 me-5 mt-1"> <i class="si-phone_drop-call"></i> </div>
                  </div>
                  <div>
                    <h5 class="mb-1">Phone</h5>
                    <p>00 (123) 456 78 90 <br />00 (987) 654 32 10</p>
                  </div>
                </div>
                <div class="d-flex flex-row">
                  <div>
                    <div class="icon text-dark fs-30 me-5 mt-1"> <i class="si-mail_send-arrow"></i> </div>
                  </div>
                  <div>
                    <h5 class="mb-1">E-mail</h5>
                    <p class="mb-0"><a href="mailto:#" class="link-body"></a></p>
                  </div>
                </div>
              </div>
              <!--/column -->
            </div>
            <!--/.row -->
            <h2 class="mt-10 mb-8 text-center">Siguenos</h2>
            <div class="grid grid-view position-relative mb-10">
              <div class="position-absolute mt-3" style="top:50%; left: 50%; z-index:2; transform: translate(-50%,-50%);"><a href="#" class="btn btn-dark"><i class="uil uil-instagram fs-18 me-1"></i> Follow Me</a></div>
              <div class="row gx-md-6 gy-6 isotope opacity-75">
                <div class="item col-md-6 col-lg-3">
                  <figure class="rounded"><img src="./assets/img/photos/i1.jpg" srcset="./assets/img/photos/i1@2x.jpg 2x" alt="" /></figure>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-lg-3">
                  <figure class="rounded"><img src="./assets/img/photos/i2.jpg" srcset="./assets/img/photos/i2@2x.jpg 2x" alt="" /></figure>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-lg-3">
                  <figure class="rounded"><img src="./assets/img/photos/i3.jpg" srcset="./assets/img/photos/i3@2x.jpg 2x" alt="" /></figure>
                </div>
                <!-- /.item -->
                <div class="item col-md-6 col-lg-3">
                  <figure class="rounded"><img src="./assets/img/photos/i4.jpg" srcset="./assets/img/photos/i4@2x.jpg 2x" alt="" /></figure>
                </div>
                <!-- /.item -->
              </div>
              <!-- /.row -->
            </div>
            <!-- /.grid -->
            <p class="mb-0 text-center">© 2022 TramitaLaVisa. All rights reserved.</p>
          </div>
          <!--/.card-body -->
        </div>
        <!--/.card -->
      </div>
      <!--/.container -->
    </section>
    <div class="swiper-container swiper-bg" data-effect="fade" data-margin="0" data-autoplay="true" data-autoplaytime="7000">
      <div class="swiper">
        <div class="swiper-wrapper">
          <div class="swiper-slide"><img src="/dev/assets/img/visa/usa2.jpg" alt="" /></div>
          <div class="swiper-slide"><img src="/dev/assets/img/visa/usa3.jpg" alt="" /></div>
          <div class="swiper-slide"><img src="/dev/assets/img/visa/usa4.jpg" alt="" /></div>
          <div class="swiper-slide"><img src="/dev/assets/img/visa/usa5.jpg" alt="" /></div>
        </div>
        <!--/.swiper-wrapper -->
      </div>
      <!-- /.swiper -->
    </div>
    <!-- /.swiper-container -->
  </div>
  <!-- /.content-wrapper -->
  <script src="./assets/js/plugins.js"></script>
  <script src="./assets/js/theme.js"></script>
  <script src="./assets/js/preloader.js"></script>
  <script src="/assets/js/download.js"></script>
</body>

</html>